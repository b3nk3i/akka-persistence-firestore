...
import org.b3nk3i.akka.persistence.firestore.SimpleSpec
import org.b3nk3i.akka.persistence.firestore.query.EventAdapterTest.{Event, TaggedAsyncEvent, TaggedEvent}
import org.b3nk3i.akka.persistence.firestore.query.javadsl.{FirestoreReadJournal => JavaFirestoreReadJournal}
...
class ScalaFirestoreReadJournalOperations(readJournal: FirestoreReadJournal)(implicit
  system:                                              ActorSystem,
  mat:                                                 Materializer
) extends ReadJournalOperations {
  def this(system: ActorSystem) =
    this(PersistenceQuery(system).readJournalFor[FirestoreReadJournal](FirestoreReadJournal.Identifier))(
      system,
...
class JavaDslFirestoreReadJournalOperations(readJournal: javadsl.FirestoreReadJournal)(implicit
  ...
  def this(system: ActorSystem) =
    this(
      PersistenceQuery
        .get(system)
        .getReadJournalFor(classOf[javadsl.FirestoreReadJournal], JavaFirestoreReadJournal.Identifier)
...
abstract class QueryTestSpec() extends SimpleSpec {
  ...
  def withTestActors(seq: Int = 1, replyToMessages: Boolean = false)(
    f:                    (ActorRef, ActorRef, ActorRef) => Unit
